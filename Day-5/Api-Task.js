...
//     let filteredProducts = data.products.filter(item => item.price > 50);
//     let res = filteredProducts.map(item => "First " + item.title);
...
// try{  
//    let name =  await fetch("https://restcountries.com/v3.1/all?fields=name");
//    let data = await name.json();
//    let res = data.map(item => " Hello " + item.name.common );
...
//     res= res.filter(p=> p.userId === 2)
//     res = res.map(p => p.title);
//     console.log(res);
//     } catch(err){
//         console.log(err);
//     }
...
//     try {
//         let res = await fetch("https://dogapi.dog/api/v2/facts");
